		...
		optionsBuilder.addPage(builder.createOptionPage()
			...
			.addOptionGroup(builder.createOptionGroup()
				...
				.addOption(builder.createBooleanOption(Identifier.parse("gnetum:fast_blit"))
					.setName(Component.translatable("gnetum.config.fastBlit"))
					.setTooltip(Component.translatable("gnetum.config.fastBlit.tooltip"))
					.setStorageHandler(this.handler)
					.setBinding(consume(Gnetum.config.fastFboBlit), supply(Gnetum.config.fastFboBlit))
					.setDefaultValue(true)
					.setEnabledProvider(cs -> cs.readBooleanOption(enabledOption), enabledOption)
					.setImpact(OptionImpact.MEDIUM)
				)
