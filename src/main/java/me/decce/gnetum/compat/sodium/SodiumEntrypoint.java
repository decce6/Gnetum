		...
		optionsBuilder.addPage(builder.createOptionPage()
			...
			.addOptionGroup(builder.createOptionGroup()
				...
				.addOption(builder.createIntegerOption(Identifier.parse("gnetum:max_fps"))
					...
					.setValueFormatter(v -> v == Constants.UNLIMITED_FPS ? Component.translatable("options.framerateLimit.max") : Component.literal(v + " fps"))
					.setDefaultValue(60)
					.setEnabledProvider(cs -> cs.readBooleanOption(enabledOption), enabledOption)
					.setBinding((i) -> Gnetum.config.maxFps = i, () -> Gnetum.config.maxFps)
					.setImpact(OptionImpact.MEDIUM)
				)
			)
		)
